...
class VideoSettings:
    ...
    height: int = 1128 # pixels
    width: int = 768 # pixels
    text_height: int = 100 # pixels
...
def write_text(content, width, height, xpos, ypos):
    text = Image.new("RGB", (width, height), color='white')
    draw = ImageDraw.Draw(text)
    draw.text((xpos, ypos), content, fill="black", font=subheadFont)
...
def composite_frame(gen, discrim, faces, image_number, settings: VideoSettings):
    ...
    inputs = [
        ...
        np.asarray(write_text("Generated Faces", settings.width, settings.text_height, 240, 30)),
        inputs[0],
        np.asarray(write_text("Generator Last Layer", settings.width, settings.text_height, 240, 30)),
        inputs[1],
...
def make_video(settings: VideoSettings, path, glob):
    ...
    for i in frame_number:
        ...
        image_number = 100_000_000 - int(str(checkpoint.stem).split("]")[-1].split("_")[0])
...
if __name__ == '__main__':
    for layers in [8, 32, 64]:
        settings = VideoSettings(f"first_layer_{layers}.mp4", duration=25, hold_at_end=5)
